...
public:
    client_call_gate(){
        ...
        protcolcall_set.insert(std::make_pair("disconnect_hub", std::bind(&client_call_gate::disconnect_hub, this, std::placeholders::_1)));
    ...
    boost::signals2::signal<void(std::string, std::string) > sig_connect_hub;
    ...
    boost::signals2::signal<void(std::string, std::string) > sig_disconnect_hub;
    void disconnect_hub(std::shared_ptr<std::vector<boost::any> > _event){
        sig_disconnect_hub(
            boost::any_cast<std::string>((*_event)[0]), 
            boost::any_cast<std::string>((*_event)[1]));
    }
